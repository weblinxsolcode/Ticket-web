...
export default function Login() {
    ...
    if (!isValidEmail(email)) {
      ...
      if (password.split("").length == 0) {
        ...
        await axios.post(`${config.baseURL}/login.php`, {
          ...
          if (getData.status == "account_block") {
        
            seterror({ variant: "danger", data: getData.data })
          }
          if(getData.status == "Password_not_match"){
           
            seterror({ variant: "danger", data: getData.data }); 
          
          }
          if(getData.status == "email_not_match"){
            seterror({ variant: "danger", data: getData.data });
  ...
  return (
    <div>
      ...
      <section className="section_breadcrumb bg_navy_blue" data-z-index={1} data-parallax="scroll" 
      >
        <Header />
          ...
          <div className="row">
            <div className="col-lg-12">
              ...
              <div className="text-center">
                <span className="animation animated fadeInUp" data-animation="fadeInUp" data-animation-delay="0.6s" style={{ animationDelay: '0.6s', opacity: 1 }}>Don't have an account?<Link to='/register/refer'> Register</Link></span>
              </div>
            </div>
          </div>
        </div>
